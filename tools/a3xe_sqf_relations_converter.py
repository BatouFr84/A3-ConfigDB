#!/usr/bin/env python3
"""PUB043 SQF converter exposing validated native relations."""
...
from tools.a3xe_sqf_resolved_converter import convert_capture_with_resolved_properties


def convert_capture_with_relations(
    capture: Mapping[str, Any],
) -> tuple[dict[str, Any], dict[str, Any]]:
    try:
        package, envelope = convert_capture_with_resolved_properties(capture)
        root = str(capture["root"])
        roots = package["snapshot"]["roots"]
        resolved = {
            root: envelope["resolvedProperties"]["classes"],
        }
        relations = build_native_relations(roots, resolved)
    except (A3XENativeRelationError, KeyError, TypeError, ValueError) as exc:
        if isinstance(exc, A3XESQFCaptureError):
            raise
        raise A3XESQFCaptureError(str(exc)) from exc

    envelope["selection"]["relationMode"] = "known_fields"
    envelope["nativeRelations"] = relations
...
def write_conversion(capture_path: str | Path, output_dir: str | Path) -> tuple[Path, Path]:
    ...
    package, envelope = convert_capture_with_relations(capture)
...
def main() -> int:
    ...
    print("A3XE_SQF_RELATION_CONVERSION=PASS")
