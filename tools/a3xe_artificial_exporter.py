...
def _resolved_properties(classes: Mapping[str, Any], classname: str) -> dict[str, Any]:
    if classname not in classes:
        raise A3XEArtificialExportError(f"unknown class: {classname}")
    ...
    while current is not None:
        ...
        if parent is not None and not isinstance(parent, str):
            raise A3XEArtificialExportError(f"invalid parent for: {current}")
        current = parent
...
def export_artificial(source: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for root_name in sorted(roots_source):
        ...
        for classname in sorted(source_classes):
            ...
            local_properties = dict(item.get("properties", {}))
    ...
    # The same validator used by the Browser must accept the generated package.
...
def main() -> int:
    ...
    print(f"A3XE_ARTIFICIAL_EXPORT=PASS")
