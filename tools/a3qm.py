#!/usr/bin/env python3
"""A3QM v0.1 normalized query model for A3-ConfigDB."""
...
def normalize_query(payload: Mapping[str, Any]) -> A3QMQuery:
    ...
    unknown = set(payload) - {"root", "filters", "limit"}
    if unknown:
    ...
    for index, item in enumerate(raw_filters):
        ...
        if unknown_filter:
            raise A3QMError(
                f"filters[{index}] contains unknown fields: {', '.join(sorted(unknown_filter))}"
            )
    ...
    return A3QMQuery(root=root, filters=tuple(filters), limit=limit)
